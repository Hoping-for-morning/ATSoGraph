...
def sampling(dataset, Global):
    Global._user_input, Global._item_input_pos = [], []
    for (u, i) in dataset.trainMatrix.keys():
        # positive instance
        Global._user_input.append(u)
        Global._item_input_pos.append(i)
    return Global._user_input, Global._item_input_pos


def shuffle(samples, batch_size, dataset, model, Global):
    Global._user_input, Global._item_input_pos = samples
    Global._batch_size = batch_size
    Global._index = range(len(Global._user_input))
    Global._model = model
    _dataset = datase
    np.random.shuffle(_index)
    num_batch = len(Global._user_input) // _batch_size
    pool = Pool(cpu_count())
