...
# output: [_user_input_list, _item_input_pos_list]
    def sampling(self, dataset):
        self._user_input, self._item_input_pos = [], []
        for (u, i) in dataset.trainMatrix.keys():
            # positive instance
            self._user_input.append(u)
            self._item_input_pos.append(i)
        return self._user_input, self._item_input_pos


    def shuffle(self, samples, batch_size, dataset, model):
        self._user_input, self._item_input_pos = samples
        self._batch_size = batch_size
        self._index = range(len(self._user_input))
        self._model = model
        self._dataset = dataset
        np.random.shuffle(self._index)
        num_batch = len(self._user_input) // self._batch_size
        pool = Pool(cpu_count())
        res = pool.map(self._get_train_batch, range(num_batch))
        pool.close()
        pool.join()
    ...
    def _get_train_batch(self, i):
        ...
        for idx in range(begin, begin + self._batch_size):
            ...
            item_batch.append(self._item_input_pos[self._index[idx]])
            for dns in range(self._model.dns):
                user = self._user_input[self._index[idx]]
                user_neg_batch.append(user)
                # negtive k
                gtItem = self._dataset.testRatings[user][1]
                j = np.random.randint(self._dataset.num_items)
                while j in self._dataset.trainList[self._user_input[self._index[idx]]]:
                    j = np.random.randint(self._dataset.num_items)
    ...
    # training
    def training(self, model, dataset, args, epoch_start, epoch_end, time_stamp):  # saver is an object to save pq
        with tf.Session() as sess:
            ...
            eval_feed_dicts = self.init_eval_model(model, dataset)

            # sample the data
            samples = self.sampling(dataset)
            ...
            for epoch_count in range(epoch_start, epoch_end+1):
                ...
                batch_begin = time()
                batches = self.shuffle(samples, args.batch_size, dataset, model)
                batch_time = time() - batch_begin

                # compute the accuracy before training
                prev_batch = batches[0], batches[1], batches[3]
                _, prev_acc = self.training_loss_acc(model, sess, prev_batch, output_adv=0)

                # training the model
                train_begin = time()
                train_batches = self.training_batch(model, sess, batches, args.adver)
                train_time = time() - train_begin

                if epoch_count % args.verbose == 0:
                    _, ndcg, cur_res = self.output_evaluate(model, sess, dataset, train_batches, eval_feed_dicts,
    ...
    def output_evaluate(self, model, sess, dataset, train_batches, eval_feed_dicts, epoch_count, batch_time, train_time, prev_acc,
                        output_adv):
        loss_begin = time()
        train_loss, post_acc = self.training_loss_acc(model, sess, train_batches, output_adv)
        loss_time = time() - loss_begin

        eval_begin = time()
        result = self.evaluate(model, sess, dataset, eval_feed_dicts, output_adv)
    ...
    def training_batch(self, model, sess, batches, adver=False):
    ...
    def adv_update(self, model, sess, train_batches):
    ...
    def training_loss_acc(self, model, sess, train_batches, output_adv):
        train_loss = 0.0
        acc = 0
    ...
    def init_eval_model(self, model, dataset):
        begin_time = time()
        self._dataset = dataset
        self._model = model

        pool = Pool(cpu_count())
        feed_dicts = pool.map(self._evaluate_input, range(self._dataset.num_users))
        pool.close()
        pool.join()
    ...
    def _evaluate_input(self, user):
        # generate items_list
        test_item = self._dataset.testRatings[user][1]
        item_input = set(range(self._dataset.num_items)) - set(self._dataset.trainList[user])
    ...
    def evaluate(self, model, sess, dataset, feed_dicts, output_adv):
        self._dataset = dataset
        self._model = model
        self._sess = sess
        self._K = 100
        self._feed_dicts = feed_dicts
        self._output = output_adv

        res = []
        for user in range(self._dataset.num_users):
    ...
    def _eval_by_user(self, user):
        ...
        user_input, item_input = self._feed_dicts[user]
        feed_dict = {self._model.user_input: user_input, self._model.item_input_pos: item_input}
        if self._output:
            predictions = self._sess.run(self._model.output_adv, feed_dict)
        else:
            predictions = self._sess.run(self._model.output, feed_dict)
    ...
    def init_logging(self, args, time_stamp):
...
if __name__ == '__main__':
    ...
    args = parse_args()
    trainer = Train()
    trainer.init_logging(args, time_stamp)
    ...
    # start training
    trainer.training(MF_BPR, dataset, args, epoch_start=0, epoch_end=args.adv_epoch-1, time_stamp=time_stamp)
    ...
    # start training
    Train.training(AMF, dataset, args, epoch_start=args.adv_epoch, epoch_end=args.epochs, time_stamp=time_stamp)
